...
type paymentList = (array<hoc>, array<React.element>)

let useSheetListModifier = () => {
  let (allApiData, _) = React.useContext(AllApiDataContext.allApiDataContext)
  let dummyFn = ((~sessionObject as _,~resolve as _)=>(), (~sessionObject as _)=>())
  let (addApplePay, addGooglePay) = ReactNative.Platform.os === #web ? WebButtonHook.usePayButton() : dummyFn
  let samsungPayStatus = SamsungPay.Invalid //SamsungPay.useSamsungPayValidityHook()

  React.useMemo1(() => {
    allApiData.paymentMethodList->Array.reduce(([], []), (
      (tabArr, elementArr): paymentList,
      paymentMethodData,
    ) => {
      if paymentMethodData.payment_method === WALLET {
        ...
        | Some(_) =>
          elementArr->Array.push(
            <PaymentMethod key=paymentMethodData.payment_method_type paymentMethodData methodType=ELEMENT />,
          )
        | None => ()
        }
      } else {
