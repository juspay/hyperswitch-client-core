...
let make = (
  ...
  let getScanCardComponent = (isScanCardAvailable, cardBrand, cardNumber, cardNetworks) => {
    CustomInput.CustomIcon(
      <View style={array([viewStyle(~flexDirection=#row, ~alignItems=#center, ())])}>
        <CardSchemeComponent cardNumber cardBrand cardNetworks />
    ...
    <View style={viewStyle(~width=100.->pct, ~borderRadius, ())}>
      <View style={viewStyle(~width=100.->pct, ())}>
        <CustomInput
          ...
          iconRight={getScanCardComponent(ScanCardModule.isAvailable, cardBrand, cardNumber, cardNetworks)}
          onFocus={() => {
