...
let make = (
  ...
  let changeData = (key, value) => {
    let requiredFieldPath = Array.find(requiredFields, (
      item: RequiredFieldsTypes.required_fields_type,
    ) => item.field_type === key)

    setFinalJson(prev =>
      prev->Array.map(item => {
        let (internalKey, _, errorText) = item

        switch (internalKey, requiredFieldPath) {
        | (internalKey, Some(requiredPath)) =>
          switch internalKey ===
            switch requiredPath.required_field {
            | StringField(val) => val
            | _ => ""
            } {
          | true => (internalKey, value, errorText)
          | false => item
          }
        | _ => item
        }
  ...
  let onScanCard = (
    ...
    let expiryValues = formatCardExpiryValues(expiry)
    let (month, year) = switch expiryValues {
    | [month, year] => (month, year)
    | [] => ("", "")
    | _ => ("", "")
    ...
    changeData(CardNumber, cardNumber->JSON.Encode.string)
    changeData(CardExpMonth, month->JSON.Encode.string)
    changeData(CardExpYear, year->JSON.Encode.string)
