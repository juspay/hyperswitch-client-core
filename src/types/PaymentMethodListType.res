...
module Parsers = {
  ...
  let parsePaymentMethodType = (paymentMethodDict: Js.Dict.t<JSON.t>, paymentMethodTypeDict: Js.Dict.t<JSON.t>) => {
...
module PaymentMethodProcessor = {
  ...
  let processPaymentMethods = (jsonArray: array<JSON.t>) => {
    let resultMap = jsonArray->Array.reduce(Belt.Map.String.empty, 
      (resultMap, item) => {
        let paymentMethodDict = item->getDictFromJson
        let paymentMethodTypes = paymentMethodDict->getArray("payment_method_types")
        
        paymentMethodTypes->Array.reduce(resultMap, 
          (accMap, paymentMethodType) => {
            ...
            let normalizedPaymentMethodType = normalizeCardType(parsed.payment_method_str, parsed.payment_method_type)
            ...
            | None => 
              accMap->Belt.Map.String.set(key, normalizedParsed)
            | Some(existing) => 
