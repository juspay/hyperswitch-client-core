import {
  ...
  NORMAL_PAYMENT_SHEET_INDICATORS,
  TIMEOUT_CONFIG,
...
export async function typeTextInInput(
  element: Detox.IndexableNativeElement,
  text: string,
) {
...
export async function waitForUIStabilization(
  duration: number = UI_STABILIZATION_WAIT,
): Promise<void> {
  const isCI = TIMEOUT_CONFIG.IS_CI;
  const waitTime = isCI
    ? Math.round(duration * TIMEOUT_CONFIG.CI_MULTIPLIER)
    : duration;
  console.log(
    `Waiting ${waitTime}ms for UI to stabilize${
      isCI ? ' (CI environment)' : ''
    }...`,
  );
...
async function findAddNewPaymentMethodElement(): Promise<Detox.IndexableNativeElement | null> {
  const variations = ['Add new payment method'];
  ...
  for (const text of variations) {
    try {
      ...
      await waitFor(element(elementMatcher))
        .toBeVisible()
        .withTimeout(ELEMENT_SEARCH_TIMEOUT);
...
export async function isSavedPaymentSheet(): Promise<boolean> {
  console.log(
    `Checking for saved payment sheet indicators${
      TIMEOUT_CONFIG.IS_CI ? ' (CI environment)' : ''
    }...`,
  );
  ...
  try {
    const titleElement = element(
      by.text(SAVED_PAYMENT_SHEET_INDICATORS.TITLE_TEXT),
    );
  ...
  try {
    ...
    if (addNewElement) {
      ...
      console.log(
        "✗ 'Add new payment method' link not found with any variation",
      );
    }
  } catch (error) {
  ...
  try {
    const savedCardElement = element(
      by.text(SAVED_PAYMENT_SHEET_INDICATORS.SAVED_CARD_PATTERN),
    );
    await waitFor(savedCardElement)
      .toBeVisible()
      .withTimeout(ELEMENT_SEARCH_TIMEOUT);
  ...
  console.log(
    `Saved payment sheet detection: ${foundIndicators}/${requiredIndicators} indicators found - ${
      isSaved ? 'CONFIRMED' : 'NOT CONFIRMED'
    }`,
  );
...
export async function isNormalPaymentSheet(): Promise<boolean> {
  console.log(
    `Checking for normal payment sheet indicators${
      TIMEOUT_CONFIG.IS_CI ? ' (CI environment)' : ''
    }...`,
  );
  ...
  try {
    const titleElement = element(
      by.text(NORMAL_PAYMENT_SHEET_INDICATORS.TITLE_TEXT),
    );
    await waitFor(titleElement)
      .toBeVisible()
      .withTimeout(ELEMENT_SEARCH_TIMEOUT);
  ...
  try {
    const cardDetailsElement = element(
      by.text(NORMAL_PAYMENT_SHEET_INDICATORS.CARD_DETAILS_TEXT),
    );
    await waitFor(cardDetailsElement)
      .toBeVisible()
      .withTimeout(ELEMENT_SEARCH_TIMEOUT);
  ...
  try {
    const cardNumberPlaceholder = element(
      by.text(NORMAL_PAYMENT_SHEET_INDICATORS.CARD_NUMBER_PLACEHOLDER),
    );
    await waitFor(cardNumberPlaceholder)
      .toBeVisible()
      .withTimeout(ELEMENT_SEARCH_TIMEOUT);
  ...
  try {
    const orPayUsingElement = element(
      by.text(NORMAL_PAYMENT_SHEET_INDICATORS.OR_PAY_USING_TEXT),
    );
    await waitFor(orPayUsingElement)
      .toBeVisible()
      .withTimeout(ELEMENT_SEARCH_TIMEOUT);
  ...
  console.log(
    `Normal payment sheet detection: ${foundIndicators}/${requiredIndicators} indicators found - ${
      isNormal ? 'CONFIRMED' : 'NOT CONFIRMED'
    }`,
  );
...
export async function navigateToNormalPaymentSheet(): Promise<void> {
  try {
    console.log(
      `Navigating from saved payment sheet to normal payment sheet${
        TIMEOUT_CONFIG.IS_CI ? ' (CI environment)' : ''
      }...`,
    );
    ...
    if (!addNewElement) {
      throw new Error(
        "Could not find 'Add new payment method' element with any variation",
      );
    }
    ...
    console.error(
      '✗ Failed to navigate to normal payment sheet:',
      error.message,
    );
...
export async function enterCardDetails(
  ...
  console.log(
    `Starting card details entry with enhanced error handling${
      TIMEOUT_CONFIG.IS_CI ? ' (CI environment)' : ''
    }...`,
  );
  await waitForUIStabilization();
...
export async function ensureNormalPaymentSheet(): Promise<'saved' | 'normal'> {
  console.log(
    `=== Payment Sheet Detection Started${
      TIMEOUT_CONFIG.IS_CI ? ' (CI Environment)' : ''
    } ===`,
  );
  ...
  try {
    ...
    if (await isSavedPaymentSheet()) {
      ...
      if (!isNormal) {
        console.log(
          '⚠️ Warning: Could not verify normal payment sheet after navigation, but proceeding...',
        );
      } else {
    ...
    throw new Error(
      'Unable to detect payment sheet type - neither saved nor normal indicators found',
    );
  } catch (error) {
